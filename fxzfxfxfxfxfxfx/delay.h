...
void processDelay(DATA *in, DATA *out, Uint16 delayLengthSampIn, DATA feedback, int length)
{
	...
	{
		delayBuffer[writeIndex] = (DATA)((((LDATA)(in[2*delayCounter]) + (LDATA)(delayBuffer[readIndex])) * (LDATA)(feedback))>>15); //write input + feedback
